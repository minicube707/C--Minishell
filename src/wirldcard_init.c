...
/*   Updated: 2025/10/26 00:31:07 by florent          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static int wildcard_init_utils21(t_shell *shell, char *buff, char **tmp4, char **path)
{   
    char	*tmp2;
...
int wildcard_init(t_shell *shell, char *string, char **path, char **expand)
{
    ...
    if (wildcard_init_utils1(shell, string, &tmp4))
        return (1);
    *expand = ft_substr(string, ft_strlen(tmp4) , ft_strlen(string) - ft_strlen(tmp4));		
    if (*expand == NULL)
    {
        free(tmp4);
        return (1);
