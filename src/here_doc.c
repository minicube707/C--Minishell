...
/*   Updated: 2025/10/23 19:04:39 by lupayet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static int	write_content(t_shell *shell)
{
	if (write(shell->fd, shell->input, ft_strlen(shell->input)) == -1)
	{
		free(shell->input);
		shell->input = NULL;
...
static int	write_here_doc(t_shell *shell)
{
