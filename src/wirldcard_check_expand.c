...
/*   Updated: 2025/10/26 17:05:57 by florent          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_expand_middle_loop(char *wilcard, char *string, char *tmp, int *i,
		int *j)
{
	int	not_ok;
...
int	check_expand_middle(char *string, char *wilcard, int i)
{
	...
	while (ft_strncmp(wild1, wild3, ft_strlen(wild1)) != 0)
	{
		while (ft_strlen(wild1) - ft_strlen(wild2) == 1)
			find_next_wildcards(wilcard, ++i, &wild1, &wild2);
		tmp = ft_substr(&wilcard[i], 1, ft_strlen(wild1) - ft_strlen(wild2)
				- 1);
